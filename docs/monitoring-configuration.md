...
- `tls_verify`：是否校验证书
- `warning_days`：证书剩余有效期不超过该天数时进入 Warning
- `critical_days`：证书剩余有效期不超过该天数时进入 Critical
...
启动时会把历史 TLS monitor 中仍为旧默认值 `warning_days: 30` 的配置回填为 `21`，避免旧默认继续让 22-30 天剩余有效期的证书进入 Warning。用户手动设置为其他值的配置不会被覆盖。
