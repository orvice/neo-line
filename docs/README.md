...
- 测试端点：`GET /ping`
- 认证 API：`POST /api/v1/auth/login`，Admin 接口通过 Bearer token 鉴权
- Server API：`/api/v1/servers`
- Monitor API：`/api/v1/servers/:id/monitors`
- Monitor Group API：`/api/v1/monitor-groups`（含分组级告警策略，引用通知组派发）
- Notify Group API：`/api/v1/notify-groups`（可复用的 webhook / Telegram / Discord / Mastodon 通知组）
...
- 记录 REST API 和 MCP 操作的审计日志（`audit_logs` collection）
