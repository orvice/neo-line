...
- 公开接口：`GET /ping`、`POST /api/v1/auth/login`，以及 server / monitor 的只读查询（`GET` 接口），方便 dashboard 读取。
- 需鉴权的 Admin 接口：server 和 monitor 的写操作（`POST` / `PUT` / `DELETE`），以及 `/api/v1/auth/me`、`/api/v1/auth/logout`。
