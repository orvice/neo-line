...
- 服务启动时通过 `MONGODB_URI` 连接 MongoDB，默认 `mongodb://localhost:27017`。
- 通过 `MONGODB_DATABASE` 指定数据库，默认 `neo_line`。
...
- `GET /v1/servers/:id/events`：`page_size`、`page_token`
...
```http
GET    /v1/monitor-groups
POST   /v1/monitor-groups
GET    /v1/monitor-groups/:group_id
PUT    /v1/monitor-groups/:group_id
DELETE /v1/monitor-groups/:group_id
GET    /v1/monitor-groups/:group_id/monitors
```
