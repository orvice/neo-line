...
- 需鉴权的 Admin 接口：除上述公开 Connect procedure 外的所有 Connect API，包括 server / monitor / monitor group / notify group / MCP token 的读写操作、SSH 远程执行、`SettingsService.UpdateSettings`、`AuthService.GetCurrentUser` 和 `AuthService.Logout`。
