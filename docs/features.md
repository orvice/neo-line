...
- REST API 与 MCP tool 调用会写入 `audit_logs` collection，用于操作审计。
...
neo-line 会为 REST API 和 MCP tool 调用记录操作日志：

- 运行日志：通过 `slog` 输出，包含来源、动作、资源类型、资源 ID、状态、耗时等字段。
- 持久化日志：写入 MongoDB `audit_logs` collection。审计写入失败只记录错误日志，不阻塞原 API 或 MCP 操作。
- REST API：覆盖 `/api/v1` 下的公开读接口与 Admin 写接口。已登录 Admin 请求会记录 `actor_id` 与 `actor_email`。
...
- `method` / `path` / `status_code`：REST API 请求信息。
- `success` / `error`：操作是否成功及错误摘要。
- `duration_ms`：操作耗时。
- `remote_ip` / `user_agent`：REST API 请求来源信息。
...
提供 API 用于管理 server 和查询 server 健康状态。Server 配置的写入、更新和删除都会落到 MongoDB。Server 列表按 `sort_order` 升序返回，值越小越靠前；排序值相同时按 `created_at` 倒序返回。
...
提供 API 用于管理分组及查询分组下的 monitor。Monitor group 配置的写入、更新和删除都会落到 MongoDB。分组列表按 `sort_order` 升序返回，值越小越靠前；排序值相同时按 `created_at` 倒序返回。
...
- 提供读和写工具，底层复用 MongoDB store，与现有 REST API 行为一致。
...
- `GET /api/v1/mcp-tokens`：列出所有 token（不含明文）。
- `POST /api/v1/mcp-tokens`：按 `name` 生成新 token，响应中包含一次性明文 `secret`。
- `DELETE /api/v1/mcp-tokens/:token_id`：吊销 token。
...
```http
GET    /api/v1/notify-groups               # 列出通知组
POST   /api/v1/notify-groups               # 创建通知组
GET    /api/v1/notify-groups/:notify_group_id    # 查询单个通知组
PUT    /api/v1/notify-groups/:notify_group_id    # 更新通知组
DELETE /api/v1/notify-groups/:notify_group_id    # 删除通知组
```
