...
- 审计日志查询通过 `AuditLogService.ListAuditLogs` 和 Web 控制台「审计」页面读取 `audit_logs` collection。
...
TLS Port 的规范类型是 `tls_port`。运行时兼容历史 MongoDB 文档中的 `tls` 和 `tls_certificate`，并按 TLS Port 探测读取证书；新建配置仍应写入 `tls_port`。
...
调度器在应用启动时通过 Butterfly `InitFunc` 拉起，在退出时通过 `TeardownFunc` 停止。停止时调度器先停止派发新探测，再等待所有 in-flight 探测 goroutine 结束（`TeardownFunc` 中以 15s 为上限）后才关闭 MongoDB 连接，避免探测在写入过程中被强制截断或写到已关闭的连接。
...
- `get_status_overview`：查询与公开状态页一致的匿名、瘦身聚合视图，按 monitor group 组织 server 和 monitor，不返回 host、URL、headers 等内部配置。
...
- `create_server` / `update_server` / `delete_server`：创建、更新、删除被监控的 server（删除 server 会级联清除其下 monitor、历史 `monitor_results` 与 `server_events`）。
...
SSH MCP 工具（仅当配置了全局 `ssh.key_path` 时才注册）：
...
**状态：** 已实现（通过 Connect API 与 MCP 工具，无限制命令执行）

neo-line 支持通过本地 SSH 私钥连接被监控的 server，并通过 Connect API 与 MCP 工具提供远程命令执行能力。私钥本身**不入库**：全局只在运行时配置中指定一个本地私钥路径，MongoDB 中的 server 文档只保存连接目标（host / port / user）等可继承、可覆盖的字段。

运行行为：

- 全局 SSH 配置来自运行时配置 `ssh`（见 `docs/monitoring-configuration.md`），其中 `ssh.key_path` 是唯一的私钥来源。`key_path` 为空时，SSH 相关 API 调用会返回未配置错误，MCP 侧不会注册 SSH 工具。
...
- `SshService.Exec`：在指定 server 上执行 shell 命令，入参 `server_id`、`command`、可选 `timeout_seconds`，返回 `host`、`stdout`、`stderr`、`exit_code`。
- `SshService.TestConnection`：对指定 server 建立 SSH 连接并执行 `echo neo-line-ssh-ok`，返回 `ok` 和输出。
- Web 控制台：服务器详情页提供「SSH 运维」tab，可对当前 server 测试连接并执行命令，展示 `stdout`、`stderr` 与 `exit_code`。

MCP 工具：

- `ssh_exec`：与 `SshService.Exec` 语义一致。
- `ssh_test_connection`：与 `SshService.TestConnection` 语义一致。

安全说明：

- 当前为**无限制**版本：`SshService.Exec` / `ssh_exec` 可执行任意命令，没有命令白名单或只读限制。访问控制依赖 Admin Bearer token 或 MCP token 鉴权，生产环境务必谨慎分发相关凭证。
- SSH API 调用和 MCP 工具调用都会写入 `audit_logs` 审计记录。
...
- 登录失败按 `(email, 来源 IP)` 维度限流：同一 key 在 15 分钟窗口内累计 5 次失败后，后续登录返回 `resource_exhausted`，登录成功立即清空计数。限流状态为单机内存态，进程重启或多副本部署不共享。
