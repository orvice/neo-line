      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 text-sm">
            <p>
              通过环境变量{" "}
              <code className="bg-muted rounded px-1">MCP_AUTH_TOKEN</code> 配置静态
              token：
            </p>
            <ul className="text-muted-foreground ml-4 list-disc space-y-1 text-sm">
              <li>
                设置后，请求需在 <code className="bg-muted rounded px-1">Authorization: Bearer &lt;token&gt;</code>{" "}
                或 <code className="bg-muted rounded px-1">X-MCP-Token: &lt;token&gt;</code>{" "}
                头中携带该 token，否则返回 <code className="bg-muted rounded px-1">401</code>。
              </li>
              <li>
                未设置时，<code className="bg-muted rounded px-1">/api/mcp</code>{" "}
                不做鉴权（仅适合受信任内网或本地开发）。
              </li>
              <li>读写工具共用同一个 token，没有更细粒度的权限区分。</li>
            </ul>
          </div>
        </CardContent>
      </Card>
